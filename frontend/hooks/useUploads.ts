...
export function useUploads(onUploaded: () => void, chunkSize?: number) {
    ...
    (files: File[]) => {
      for (const file of files) {
        ...
        const uploader = new ChunkedUploader(file, {
          chunkSize,
          onProgress: (p) => {
            patch(id, (e) => ({ ...e, ...p, paused: uploader.isPaused }));
          },
          onComplete: () => {
  ...
  const resume = useCallback((id: string) => {
    ...
    if (u) {
      void u.resume();
      patch(id, (e) => ({ ...e, paused: false }));
    }
  }, [patch]);

  const cancel = useCallback((id: string) => {
    uploaders.current.get(id)?.cancel();
    patch(id, (e) => ({ ...e, phase: "cancelled" as const }));
  }, [patch]);

  const clearFinished = useCallback(() => {
    setEntries((prev) => prev.filter((e) => e.phase !== "done" && e.phase !== "cancelled" && e.phase !== "error"));
  }, []);

  const activeCount = entries.filter(
    (e) => e.phase === "uploading" || e.phase === "preparing" || e.phase === "completing" || e.paused,
  ).length;
