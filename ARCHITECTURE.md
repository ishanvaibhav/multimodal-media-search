...
- **Query normalization/expansion**: deterministic only (no LLM). Connector
  queries are split into components and their candidates fused (`max`/`sum`).
  Query embeddings are cached in-process (LRU).
- **Temporal context**: each result is enriched with a window-bounded segment
  (`context_window_seconds`), frames within it (`frames.between`), a grounded
  reason, and a plain-text context block (`format_context_text`).
